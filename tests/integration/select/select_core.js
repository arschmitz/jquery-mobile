...
(function($) {
  ...
  asyncTest("dialog size select title should match the placeholder when there's no label", function() {
  ...
  asyncTest("destroying a custom select menu leaves no traces", function() {
    ...
    $.testHelper.detailedEventCascade([function() {
...

      {
        popupafterclose: {
          src: $.mobile.document,
          event: "popupafterclose" + prefix + "2"
        }
      }, function(result) {
        var idx;
