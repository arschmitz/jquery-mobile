...
(function($) {
  ...
  var urlObject = $.mobile.path.parseLocation(),
    ...
    opensAndCloses = function(eventNs, popupId, linkSelector, contentSelector) {
      ...
      $.testHelper.detailedEventCascade([function() {
  ...
  asyncTest("Popup interacts correctly with hashchange", function() {
    ...
    $.testHelper.detailedEventCascade([function() {
...

      {
        opened: {
          src: $("#test-popup"),
          event: "popupafteropen.hashInteractStep1"
        },
        hashchange: {
          src: $(window),
          event: "hashchange.hashInteractStep1"
        }
      }, function(result) {
...

      {
        closed: {
          src: $("#test-popup"),
          event: "popupafterclose.hashInteractStep2"
        }
      }, function(result) {
  ...
  asyncTest("Opening another page from the popup leaves no trace of the popup in history", function() {
    ...
    $.testHelper.detailedEventCascade([function() {
...

      {
        opened: {
          src: $("#test-popup"),
          event: "popupafteropen.anotherPageStep1"
        },
        hashchange: {
          src: $(window),
          event: "hashchange.anotherPageStep1"
        }
      }, function() {
        $("#test-popup a").click();
      },

      {
        closed: {
          src: $("#test-popup"),
          event: "popupafterclose.anotherPageStep2"
        },
        hashchange: {
          src: $(window),
          event: "hashchange.anotherPageStep2"
        }
      }, function(result) {
  ...
  asyncTest("Popup assures previous element is blurred", function() {
    ...
    $.testHelper.detailedEventCascade([function() {
...

      {
        focus: {
          src: textinput,
          event: "focus.popupFocusedAfterOpen0"
        }
      }, function(result) {
...

      {
        blur: {
          src: textinput,
          event: "blur.popupFocusedAfterOpen1"
        },
        opened: {
          src: popup,
          event: "popupafteropen.popupFocusedAfterOpen1"
        }
      }, function(result) {
...

      {
        focus: {
          src: textinput,
          event: "focus.popupFocusedAfterOpen2"
        }
      }, function(result) {
...

      {
        closed: {
          src: popup,
          event: "popupafterclose.popupFocusedAfterOpen2"
        }
      }, function(result) {
  ...
  asyncTest("Popup doesn't alter the url when the history option is disabled", function() {
    ...
    $.testHelper.detailedEventCascade([function() {
...

      {
        opened: {
          src: $popup,
          event: "popupafteropen.popupDoesntAlertUrl1"
        }
      }, function(result) {
...

      {
        closed: {
          src: $popup,
          event: "popupafterclose.popupDoesntAlertUrl2"
        },
        timeout: {
          length: 1000
        }
  ...
  asyncTest("Navigating away from the popup page closes the popup without history enabled", function() {
    ...
    $.testHelper.detailedEventCascade([function() {
...

      {
        open: {
          src: $popup,
          event: "popupafteropen.historyOffTestStep1"
        }
      }, function() {
        ok($popup.is(":visible"), "popup is indeed visible");
...

      {
        hashchange: {
          src: $(window),
          event: "hashchange.historyOffTestStep2"
        },
        close: {
          src: $popup,
          event: "popupafterclose.historyOffTestStep2"
        }
      }, function(result) {
  ...
  asyncTest("Close links work on a history disabled popup", function() {
    ...
    $.testHelper.detailedEventCascade([function() {
...

      {
        opened: {
          src: $popup,
          event: "popupafteropen.closeLinksWorks1"
        }
      }, function() {
...

      {
        closed: {
          src: $popup,
          event: "popupafterclose.closeLinksWorks2"
        }
      }, function() {
  ...
  asyncTest("Destroy closes open popup first", function() {
    ...
    $.testHelper.detailedEventCascade([function() {
...

      {
        opened: {
          src: $popup,
          event: "popupafteropen.destroyClosesOpenPopupFirst1"
        }
      }, function() {
        $popup.popup("destroy");
      },

      {
        closed: {
          src: $popup,
          event: "popupafterclose.destroyClosesOpenPopupFirst2"
        }
      }, function(result) {
  ...
  asyncTest("Cannot close a non-dismissible popup by clicking on the screen", function() {
    ...
    $.testHelper.detailedEventCascade([function() {
...

      {
        popupafteropen: {
          src: $popup,
          event: "popupafteropen" + eventNs + "0"
        }
      }, function(results) {
...

      {
        popupafterclose: {
          src: $popup,
          event: "popupafterclose" + eventNs + "1"
        }
      }, function(results) {
  ...
  asyncTest("Elements inside the popup lose focus when the popup is closed", function() {
    ...
    $.testHelper.detailedEventCascade([function() {
    ...
    },
      {
        popupafteropen: {
          src: $popup,
          event: "popupafteropen" + eventSuffix + "1"
        }
      }, function(result) {
      ...
      },
      {
        popupafterclose: {
          src: $popup,
          event: "popupafterclose" + eventSuffix + "3"
        }
      }, function(result) {
